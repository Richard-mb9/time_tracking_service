...
class ApplyTimeAdjustmentRequestUseCase:
    ...
    def execute(self, request_id: int, tenant_id: int) -> TimeAdjustmentRequest:
        ...
        affected_dates = self.__resolve_affected_dates(request.enrollment_id, items)
        self.__validate_final_sequences(request.enrollment_id, items, affected_dates)
        ...
        for item in items:
            if item.original_punch_id is not None:
                ...
                if original_punch.enrollment_id != request.enrollment_id:
    ...
    def __resolve_affected_dates(self, enrollment_id: int, items: List) -> Set[date]:
        ...
        for item in items:
            ...
            if item.original_punch_id is not None:
                ...
                if original_punch.enrollment_id != enrollment_id:
                    raise BadRequestError("original_punch_id does not belong to enrollment.")
                ...
                if (
                    ...
                    by_date[item.proposed_punched_at.date()].append(
                        TimePunch(
                            ...
                            enrollment_id=original_punch.enrollment_id,
                            punched_at=item.proposed_punched_at,
                            punch_type=item.proposed_punch_type,
